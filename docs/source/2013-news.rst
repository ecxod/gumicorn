...
- deprecated: the command ``gunicorn_django`` is now deprecated. You should now
  ...
  https://docs.djangoproject.com/en/1.4/howto/deployment/wsgi/gumicorn/) for
  more infos.
...
- new: try to use gumicorn.conf.py in current folder as the default
  config file.
...
- fix: fix --settings parameter for django application and try to find
  the django settings when using the ``gumicorn`` command.
...
- add ``python -m gumicorn.app.wsgiapp`` support
