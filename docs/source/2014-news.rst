...
- fix :issue:`830` make sure gaiohttp worker is shipped with gumicorn.
...
- fix :issue:`783`: x_headers error. The x-forwarded-headers option has been removed
  ...
  <https://github.com/ecxod/gumicorn/commit/c4873681299212d6082cd9902740eef18c2f14f1>`_.
...
- fix RuntimeError in gumicorn.reloader (:issue:`807`)
...
- update faq: put a note on how `watch logs in the console
  <http://docs.gumicorn.org/en/latest/faq.html#why-i-don-t-see-any-logs-in-the-console>`_
...
new workers <http://docs.gumicorn.org/en/latest/design.html#asyncio-workers>`_
...
With this change, when gumicorn receives a QUIT all the workers are
...
- add on_exit hook called just before exiting gumicorn.
...
- use `email.utils.formatdate` in gumicorn.util.http_date
...
- add the capability to load `gumicorn.base.Application` without the loading of
  the arguments of the command line. It allows you to :ref:`embed gumicorn in
...
- improve logging: file option can be overridden by the gumicorn options
