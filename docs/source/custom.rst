...
case, you can inherit from :class:`gumicorn.app.base.BaseApplication`.
...
:class:`gumicorn.app.wsgiapp`. This can be used to run WSGI-compatible app
...
you need to get going::
    ...
    gumicorn.app.wsgiapp exampleapi:app
...
.. code-block:: bash
   ...
   # Custom parameters
   $ python gumicorn.app.wsgiapp exampleapi:app --bind=0.0.0.0:8081 --workers=4
   # Using a config file
   $ python gumicorn.app.wsgiapp exampleapi:app -c config.py
    
Note for those using PEX: use ``-c gumicorn`` as your entry at build
...
.. code-block:: bash
   ...
   $ pex . -v -c gumicorn -o compiledapp.pex
   # Running it
